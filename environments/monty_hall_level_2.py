...
class MontyHallLevel2:
    def __init__(self):
        self.observation_space = Discrete(5)  # 5 doors
        ...
        for state in range(self.observation_space.n):
            ...
            for action in range(self.action_space.n):
                for winning_door in range(self.observation_space.n):
                    next_state = 0 if action == 0 else (winning_door if winning_door != state else (state + 1) % 5)
    ...
    def reset(self):
        self.state = np.random.choice([0, 1, 2, 3, 4])
        return self.state

    def step(self, action):
        winning_door = np.random.choice([0, 1, 2, 3, 4])
        next_state = 0 if action == 0 else (winning_door if winning_door != self.state else (self.state + 1) % 5)
        reward = 1.0 if next_state == winning_door else 0.0
